...
yarn cli render report/shell-tests/dest.yaml script/fixture/example-overwrite.hbs report/shell-tests/rendered.md --config '{ "inline": "Inline works"}' --no-overwrite
if [ ! -z $(grep "overwrite" "report/shell-tests/rendered.md") ]; then
    echo "❌ KO overwrite rendered.md"
    exit 1
fi
