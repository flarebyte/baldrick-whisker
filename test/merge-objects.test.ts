...
const loadFixtureAsInputs = async (fixturePath: string): Promise<InputContent[]> => {
  ...
  return data.map((entry) => {
    ...
    if (entry.fileType === 'json' || entry.fileType === 'yaml' || entry.fileType === 'csv') {
      return {
...
describe('merge-objects (from spec fixtures)', () => {
  it('should merge two files without primary key', async () => {
    const inputs = await loadFixtureAsInputs('spec/fixtures/file-content-no-primary.yaml');
    ...
    assert.ok((merged.arrayOfString as any[]).length >= 1);
  });

  it('should merge two files with primary key', async () => {
    const inputs = await loadFixtureAsInputs('spec/fixtures/file-content-with-primary.yaml');
    const merged = mergeObjects(inputs);
    const items = (merged.arrayOfObjs as any[]) || [];
  ...
  it('should include a message for a corrupted file', async () => {
    const inputs = await loadFixtureAsInputs('spec/fixtures/file-content-corrupted.yaml');
    const merged = mergeObjects(inputs);
  ...
  it('should merge an elm file with a json file', async () => {
    const inputs = await loadFixtureAsInputs('spec/fixtures/file-content-elm-and-json.yaml');
    const merged = mergeObjects(inputs);
    assert.ok('functions' in merged);
    assert.ok(Array.isArray((merged as any).functions));
  });
});
